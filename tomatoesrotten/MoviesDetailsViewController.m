...
- (void)viewDidLoad {
    ...
    self.movieSynopsisLabel.text = self.movieDetails[@"synopsis"];
    ...
    CGSize targetSize =  self.moviePosterView.bounds.size;
    NSLog(@"%f", targetSize.width);
    NSLog(@"%f", targetSize.height);
    UIImage * image = [UIImage imageWithData:image_data];
    UIGraphicsBeginImageContextWithOptions(targetSize, NO, 0.0);
    [image drawInRect:CGRectMake(0, 0, targetSize.width, targetSize.height)];
    UIImage* resized = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    [self.moviePosterView setImage:resized];
    self.moviePosterView.contentMode = UIViewContentModeScaleAspectFill;
    self.moviePosterView.clipsToBounds = YES;
